...
IO.prototype.scard = function(key, cb) {
	return r0_j0(this, 'scard', key, this.id, cb);
};
...
// return a webpage
IO.prototype.html = function(webpage) {
	if (this.resp) {
		this.resp.end(webpage);
	}
};
...
IO.prototype.end = function (evt, obj) {
	var self = this;
	var s = self.shard('', self.id);
	self.db[s.partition].ping(function(){	// sync data
		if (!obj) {
			obj = evt;
			evt = null;
		}
		...
		if (self.resp) {
			...
			share.log(self.id || share.ip(self.req), '<===', res);

			self.resp.end(res);
		} else {
			self.push(self.id, obj);	// no resp => push
		}
	});
};
