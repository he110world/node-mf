...
IO.prototype.shard = function(key, id) {
	...
	if (/^[\w\.]+$/.test(key) && id) {	// key is variable name & id is non-zero => concat id to key
...
IO.prototype.end = function (obj) {
	...
	if (this.resp) {
		this.resp.end(JSON.stringify(this.obj));
	} else {
		this.push(this.id, this.obj);	// no resp => push
	}
};

IO.prototype.nil = function() {
	if (this.resp) {
		this.resp.end();
	}
};
...
IO.prototype._push = function(id, channel, evt, msg) {
	...
	this.db[s.partition].publish(channel, msg);
};
