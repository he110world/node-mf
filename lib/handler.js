...
exports.err = function (resp, code, msg) {
	resp.statusCode = code;
	resp.statusMessage = 'err: ' + msg;
	resp.end();
};

exports.handle = function(req, resp, payload, paramstr) {
	// parse args
	var params;
	...
	var io = new IO(dblist, payload.id, req, resp, this.err);
	...
	auth.verify(req, function(err, payload) {
		...
		if (req.method === 'POST') {
			...
			req.on('data', function (data) {
				...
				if (body.length > 1e5) {
					exports.err(resp, 413);
				}
			});
			req.on('end', function () {
				try {
					exports.handle(req, resp, payload, body);
				} catch (e) {
					console.log(e.stack);
					exports.err(resp, 400);
				}
			}); 
		} else if (req.method === 'GET') {
			exports.handle(req, resp, payload);
		}
	});
}
